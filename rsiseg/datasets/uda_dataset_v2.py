...
class UDADatasetV2(object):
    ...
    def __getitem__(self, idx):
        if self.rcs_enabled:
            ...
            # idx1 = np.random.choice(range(len(self.source)))
            idx2 = np.random.choice(range(len(self.target)))
            ...
            if 'img_strong_aug' in s2:
                results['target_img_strong_aug'] = s2['img_strong_aug']

            if 'ori_img' in s2:
                results['target_img_ori'] = s2['ori_img']
