...
class WandbHookStatistics(WandbLoggerHook):
    ...
    def __init__(self,
                 ...
                 sim_feat_cfg=None,
                 **kwargs):
        super(WandbHookStatistics, self).__init__(init_kwargs, interval, **kwargs)
        self.data_cfg = data_cfg
        self.sim_feat_cfg = sim_feat_cfg

    @master_only
    def before_run(self, runner):
        super(WandbHookStatistics, self).before_run(runner)
    ...
    def log_sim_hist(self):
        Case1a_sim_hist = self.sim_hist[0] / self.sim_hist[0].sum()
        Case1b_sim_hist = self.sim_hist[1] / self.sim_hist[1].sum()
        Case2a_sim_hist = self.sim_hist[3] / self.sim_hist[3].sum()
        Case2b_sim_hist = self.sim_hist[2] / self.sim_hist[2].sum()

        chart_names = {0: 'Case 1a', 1: 'Case 1b', 3: 'Case 2a', 2: 'Case 2b'}

        wandb_dict = {}
        for idx, chart_name in chart_names.items():
            sim_hist = self.sim_hist[idx] / self.sim_hist[idx].sum()
            chart_name = chart_names[idx]
            wandb_data = []
            wandb_data = [(x_val, y_val) for x_val, y_val in enumerate(sim_hist)]
            table = self.wandb.Table(data=wandb_data, columns = ["Similarity", "Frequency"])
            wandb_dict[chart_name] = self.wandb.plot.bar(table, "label", "value", title=chart_name)
