...
class PlotMultiClassStatisticsHook(Hook):
    ...
    def before_run(self, runner):
        super(PlotMultiClassStatisticsHook, self).before_run(runner)
    ...
    def after_train_iter(self, runner):
        super(PlotMultiClassStatisticsHook, self).after_train_iter(runner)
    ...
    def _add_loc_sim(self, seg_logits, gt, feats, feat_level):
        ...
        border_margin = self.sim_feat_cfg.get('border_margin', None)
        ...
        for cls_idx in range(num_classes):
            ...
            if border_margin is not None:
                mask_cls[:, :, :border_margin, :border_margin] = False
                mask_cls[:, :, -border_margin:, -border_margin:] = False


            sim_tp = sim_feat[mask & mask_tp & mask_diag & mask_cls]
            sim_tn = sim_feat[mask & mask_tn & mask_diag & mask_cls]
            sim_fn = sim_feat[mask & mask_fn & mask_diag & mask_cls]
            sim_fp = sim_feat[mask & mask_fp & mask_diag & mask_cls]

            self.add_sim_hist([sim_tp, sim_tn, sim_fn, sim_fp], cls_idx=cls_idx)
    ...
    def add_sim_hist(self, sim_dist, cls_idx=None):
        num_bins = 20
        if not hasattr(self, 'sim_hist'):
            self.sim_hist = {}
            for k, v in self.class_labels.items():
                self.sim_hist[k] = np.zeros((4, num_bins))

        for i in range(4):
            self.sim_hist[cls_idx][i] += np.histogram(sim_dist[i].cpu().numpy(), bins = num_bins, range=(0, 1))[0]
    ...
    def plot_sim_hist(self):
        ...
        for cls_idx, cls_name in self.class_labels.items():

            fig, ax = plt.subplots()
            fig.set_size_inches(5, 3)

            ax.bar(range(num_bins), self.sim_hist[cls_idx][0] / self.sim_hist[cls_idx][0].sum(),
                       color=(228/255.0, 26/255.0, 28/255.0, 0.8), label='Case 1a')
            ax.bar(range(num_bins), self.sim_hist[cls_idx][1] / self.sim_hist[cls_idx][1].sum(),
            ...
            ax.set(title = cls_name.replace('_', ' ').capitalize())
            fig.tight_layout()
            fig.savefig(os.path.join(self.log_dir, f'{cls_name}_sim_hist_case1.png'))

            fig, ax = plt.subplots()
            fig.set_size_inches(5, 3)

            ax.bar(range(num_bins), self.sim_hist[cls_idx][3] / self.sim_hist[cls_idx][3].sum(),
                       color=(228/255.0, 26/255.0, 28/255.0, 0.8), label='Case 2a')
            ax.bar(range(num_bins), self.sim_hist[cls_idx][2] / self.sim_hist[cls_idx][2].sum(),
            ...
            ax.set(title = cls_name.replace('_', ' ').capitalize())
            fig.tight_layout()
            fig.savefig(os.path.join(self.log_dir, f'{cls_name}_sim_hist_case2.png'))
        ...
        fig.savefig(os.path.join(self.log_dir, 'local_rank.png'))
