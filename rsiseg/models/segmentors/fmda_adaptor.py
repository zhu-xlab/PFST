...
class FMDAAdaptor(BaseSegmentor):
    ...
    def __init__(self,
        ...
        self.pre_feat_shape = pre_feat_shape
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        for feats in feats_list:
            ...
            for data, metas in zip(feats, img_metas_trg):
                if self.pre_feat_shape is not None:
                    data = F.interpolate(data.unsqueeze(0), self.pre_feat_shape)[0]

                c, h, w = data.shape
                data = self.transform_by_metas(data.unsqueeze(0), metas, scale=h / ori_H)
                cur_aug_feats_list.append(data.squeeze(0))

            cur_aug_feats = torch.stack(cur_aug_feats_list, dim=0)
            aug_feats_list.append(cur_aug_feats)
        ...
        loss_sim, state_sim_feat = self.loss_sim_feat(aug_feats_list, state_dec_trg['seg_logits'])
