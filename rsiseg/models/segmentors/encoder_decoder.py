...
class EncoderDecoder(BaseSegmentor):
    ...
    def encode_decode(self, img, img_metas):
        ...
        out, states = self._decode_head_forward_test(x, img_metas)
        out = resize(
            input=out,
            size=img.shape[2:],
            mode='bilinear',
            align_corners=self.align_corners)
        states.update({'feats': x, 'seg_logits': out})
    ...
    def _decode_head_forward_test(self, x, img_metas):
        """Run forward function and calculate loss for decode head in
        inference."""
        seg_logits, states = self.decode_head.forward_test(x, img_metas, self.test_cfg)
        return seg_logits, states
    ...
    def _auxiliary_head_forward_train(self, x, img_metas, gt_semantic_seg, seg_weight=None):
        ...
        if isinstance(self.auxiliary_head, nn.ModuleList):
            for idx, aux_head in enumerate(self.auxiliary_head):
                loss_aux, state_aux = aux_head.forward_train(
                    x, img_metas, gt_semantic_seg, self.train_cfg, seg_weight)
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        Returns:
            dict: It should contain at least 3 keys: ``loss``, ``log_vars``,
                ...
                averaging the logs.
        """
        losses, states = self(**data_batch, return_states=True)
    ...
    def forward_train(self, img, img_metas,
        ...
        loss_decode, state_decode = self._decode_head_forward_train(
        ...
        if return_logits:
            losses['logits'] = state_decode['seg_logits']

        if return_decoded_feats:
            losses['decoded_features'] = state_decode['decoded_features']
    ...
    def inference(self, img, img_meta, rescale):
        ...
        if flip:
            ...
            if type(flip_direction) != list:
                flip_direction = [flip_direction]

            for direction_ in flip_direction:
                assert direction_ in ['horizontal', 'vertical']
                if direction_ == 'horizontal':
                    output = output.flip(dims=(3, ))
                elif direction_ == 'vertical':
    ...
    def simple_test(self, img, img_meta, rescale=True):
        ...
        for idx in range(len(seg_pred)):
            ...
            if 'decoded_features' in states:
                cur_state['decoded_feats'] = states['decoded_features'].cpu()
    ...
    def aug_test(self, imgs, img_metas, rescale=True):
        ...
        seg_logit, states = self.inference(imgs[0], img_metas[0], rescale)
