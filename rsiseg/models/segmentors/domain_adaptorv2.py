...
class DomainAdaptorV2(BaseSegmentor):
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 weight_trg=1.0):
        super(DomainAdaptorV2, self).__init__(init_cfg)
        if pretrained is not None:
            assert backbone.get('pretrained') is None, \
                'both backbone and segmentor set pretrained weight'
            backbone.pretrained = pretrained
        self.backbone = builder.build_backbone(backbone)
        if aux_seg_net is not None:
            self.aux_seg_net = builder.build_backbone(aux_seg_net)
        else:
            self.aux_seg_net = None
    ...
    def _get_aux_losses(self, tensors):
        ...
        loss, log_vars = self._parse_losses(aux_losses)
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        tensors = dict(
            img_src = img_src,
            img_metas_src = img_metas_src,
            img_trg = img_trg,
            img_metas_trg = img_metas_trg,
            gt_src = gt_src,
            gt_trg = gt_trg,
            x_src = x_src,
            x_trg = x_trg,
            ...
            aux_seg_net = self.aux_seg_net
        )

        """ auxilary loss """
        loss_aux, log_var_aux = self._get_aux_losses(tensors=tensors)
        log_vars.update(log_var_aux)

        """ Total loss """
        loss_total = loss_dec_src + loss_aux
        if self.with_auxiliary_head:
            loss_total += loss_aux_src

        """ Record the states """
        states = {
            'vis|seg_mask_src': (img_src, gt_src,
