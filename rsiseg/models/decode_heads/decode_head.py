...
class BaseDecodeHead(BaseModule, metaclass=ABCMeta):
    ...
    def forward_train(self, inputs, img_metas, gt_semantic_seg, train_cfg, seg_weight=None):
        ...
        # decoded_features = self._forward_feature(inputs)
        # seg_logits = self.cls_seg(decoded_features)

        seg_logits, decoded_features = self.forward(inputs, return_features=True)
        gt_semantic_seg = gt_semantic_seg.type(torch.LongTensor).cuda()

        losses = self.losses(seg_logits, gt_semantic_seg, seg_weight)

        state = {'seg_logits': seg_logits,
    ...
    def forward_test(self, inputs, img_metas, test_cfg):
        ...
        state = {'decoded_features': decoded_features}
