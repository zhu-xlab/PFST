...
@UDA.register_module()
class PGSTMixFeat(UDADecorator):

    def __init__(self, **cfg):
        super(PGSTMixFeat, self).__init__(**cfg)
        self.local_iter = 0
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, target_img,
        ...
        # Apply mixing
        mixed_img, mixed_lbl, mixed_img_weak = [None] * batch_size, [None] * batch_size, [None] * batch_size
        ...
        for i in range(batch_size):
            ...
            mixed_img_weak[i], _ = strong_transform(
                strong_parameters,
        ...
        mixed_img_weak = torch.cat(mixed_img_weak)
        # mixed_ema_feats = torch.cat(mixed_ema_feats)
        # mixed_ema_feats = [None] * self.feat_level + [mixed_ema_feats]

        mixed_ema_logits, mixed_ema_states = self.get_ema_model().encode_decode(
            mixed_img_weak, target_img_metas)
        mixed_ema_feats = mixed_ema_states['feats']
