...
@UDA.register_module()
class PGSTTRG(UDADecorator):

    def __init__(self, **cfg):
        super(PGSTTRG, self).__init__(**cfg)
        self.local_iter = 0
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, target_img,
        ...
        #         strong_parameters,
        #         data=target_img_strong_aug[i:i+1],
        ...
        trg_losses = self.get_model().forward_train(
            target_img_strong_aug, target_img_metas, pseudo_label, pseudo_weight, return_feats=True, return_logits=True)
        trg_logits = trg_losses.pop('logits')
        ...
        vis_states.update({
            ...
            'vis|seg_mask_trg': (target_img, pseudo_label, trg_logits.max(dim=1)[1].unsqueeze(1)),
            'vis|density_pseudo': (target_img, pseudo_weight.unsqueeze(1) / ps_size)
        })
