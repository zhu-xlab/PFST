...
from rsiseg.models.utils.pfst_transforms import transform_by_metas
...
@UDA.register_module()
class PFSTV4(UDADecorator):

    def __init__(self, **cfg):
        super(PFSTV4, self).__init__(**cfg)
        self.local_iter = 0
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, target_img,
                      target_img_metas, target_img_strong_aug, target_img_ori):
        ...
        ema_logits, ema_states = self.get_ema_model().encode_decode(
            target_img_ori, target_img_metas)
        ema_feats = ema_states['feats']

        for i in range(len(target_img_metas)):
            ema_logits[i:i+1] = transform_by_metas(
                ema_logits[i:i+1], target_img_metas[i], scale=1.)

            ema_feats[self.feat_level][i:i+1] = transform_by_metas(
                ema_feats[self.feat_level][i:i+1], target_img_metas[i], scale=1/8.)
