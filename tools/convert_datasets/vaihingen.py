...
def clip_big_image(image_path, clip_save_dir, to_label=False):
    ...
    if to_label:
        # color_map = np.array([[255, 255, 255], [0, 0, 255], [0, 255, 255], [0, 255, 0], [255, 255, 0], [255, 0, 0]])
...
def main():
    ...
    with tempfile.TemporaryDirectory(dir=args.tmp_dir) as tmp_dir:
        for zipp in zipp_list:
            ...
            elif 'ISPRS_semantic_labeling_Vaihingen_ground_truth_COMPLETE.zip' in zipp:  # noqa
                zip_file.extractall(os.path.join(tmp_dir,'gt'))
                src_path_list = glob.glob(os.path.join(tmp_dir,'gt','*.tif'))
